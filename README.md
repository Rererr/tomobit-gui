...
- [ADR-0014](docs/decisions/ADR-0014-speech-and-work-are-separated.md) — Tomoが喋ったことと、Tomoがやったことを分ける（本人指摘「余計なものがtomoからの返答に見えすぎている / tomoが喋ったことと処理ログをできる限り分けたい」。問題は情報量ではなく**発話と作業の記録が同じ器の同じ縦の並びに混ざっている**こと＝`foldWorkBlocks` は連続 run 単位で畳んでいたので、**答えを前に出すために畳んだものが本文を分断していた** / D1 会話は左寄せ1列・寄せと色ではなく**名前**で誰の声かを言う〈Tomo側の器を外すのは中身が発言だけではないから＝`min(max(640px,90%),100%)` という幅の式はその無理の跡。話者ラベルは sr-only をやめて目にも出す＝目と耳の表示が一致する。連続する同じ話者では名前行だけ省き、読み上げ用ラベルは残す〉 / **却下: 時刻を出す**〈view はターンの時刻を持たず、GUIの時計で埋めると過去セッションの再生と食い違う＝ライブでは時刻があり開き直すと消える。ADR-0001 D1「新しい真実を作らない」〉 / D2 終わったターンの作業は**ターンの末尾へ1本に集約**〈走行中は畳まない＝ADR-0008 のまま・中身は1つも捨てない・error は畳まない・text の無いターンは畳まない。**捨てたものを正直に書く**＝「この本文の後にこの作業をした」の対応は既定の表示から消える〉 / 却下: 右ドロワーへ完全分離〈面が増えると「どの返答の作業か」の対応付けを新しく作る羽目になる＝今 turn が無料で持っている情報〉 / D3 provider・判断の監査・所要時間・コストをその1行へ合流〈常時チップと常時トグルと帳簿の行が答えの周りに3つ出ていた〉 / D4 反応は返答の隣に置く＝本体 ADR-0057 の `/react` を Slack のリアクションの形で出す〈語彙は `init` から読む＝GUIは語を持たない・押した通りには描かず本体の `reaction` で確定・置けるのは**いま開いているタスクの終わったターンだけ**・分割の子には出さない〉。**画面の印はタスクにつき高々1つ**〈本体の締めが読むのは最後の1件だけなので、2つ見えていて効くのは片方、という画面は記録される内容について嘘をつく。履歴は台帳のイベント列が持つ〉。**置き先は同じ n を名乗る枠の最後**〈畳み戻しでは結論が2つ目の枠に着地する〉。**末尾のメッセージだけホバー無しでも出す**〈実測でホバー前は地と1ピクセルも差が出ない＝気づかれない反応は締めが軽くなる設計意図ごと発火しない〉。**問いが立っている間は送らない**〈本体は1本の stdin を順に読むので、走行中に送った行が権限の問いの答えとして飲まれる＝反応も権限も両方消える。外部レビューでもP1として確認。溜めて口が空いてから送る。**入力欄側の同じ危険は塞いでいない** — BACKLOG へ〉 / **締めの1枚はGUIが何もせずに軽くなる**〈反応があれば本体が訊かない＝ADR-0005/0012 の1枚は1バイトも変えない。器官の位置を本体が持っていることの帰結〉 / D5 過去セッションでも反応は見えるが置けない〈同じ画面に評価器官が2つ並ぶが層が違う——反応は一次情報、ADR-0010 の判定は機械の観測への拒否権〉。Accepted・実装済み）
