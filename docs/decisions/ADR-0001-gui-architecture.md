...
### 追記（2026-07-26・顔窓の既定を OFF へ反転）

`face_enabled` のキー無し既定を **ON から OFF へ**反転する。理由は一つ:
上の追記でサイドバーに姿が立った以上、顔窓も既定で開けば**同じ Tomo が
画面に二匹並ぶ**。実機で最初に出た報告がそれだった（端末から `tomobit-gui`
を開くと顔窓も開き、二匹になる）。

- 「姿の正本は顔窓」という構造は動かさない。反転したのはレンダラの**枚数**の
  既定であって、正本の所在ではない。この Decision 自体は生きている
- 別窓に浮かぶ相棒が欲しい人は設定の「顔窓を開く」を入れる（明示 true）。
  既に明示 true が書かれた `gui.json` はそのまま顔窓が開く —— 既定の反転が
  効くのは「まだ何も言っていない人」にだけで、書かれた意思は覆さない
  （env>config の序列を GUI 設定にも延ばした形）
- tri-state（`*bool`）は残す。キー無しと明示 false は同じ OFF に落ちるが、
  切った意思は `gui.json` に残しておく。OFF の表現が `=0` ではなく**沈黙**で
  あることは 2026-07-24 の追記のまま変わらない
- 端末の `tomobit`（TTY起動）側の既定は本体 ADR-0025 のままで、ここでは
  何も動かない —— 顔窓が唯一の姿である入口で、姿を消す理由は無い
